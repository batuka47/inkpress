...
export default function CategoriesManager({ categories: initial }: { categories: Category[] }) {
  ...
  const [editNameMn,  setEditNameMn]  = useState("");
  ...
  async function save(id: string) {
    const { data } = await db().from("categories")
      .update({ name: editName.trim(), name_mn: editNameMn.trim() || null, slug: slugify(editName), description: editDesc.trim() || null })
      ...
      <div className="bg-white border border-[--color-rule] rounded-xl p-5 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <input value={name} onChange={(e) => setName(e.target.value)}
            placeholder="Name in English (e.g. Politics)" className={fieldCls} />
          <input value={nameMn} onChange={(e) => setNameMn(e.target.value)}
            placeholder="Нэр монгол хэлээр (жнь. Улс төр)" className={fieldCls} />
          <input value={description} onChange={(e) => setDescription(e.target.value)}
            placeholder="Description (optional)" className={`col-span-2 ${fieldCls}`} />
        </div>
              ...
              <>
                ...
                <div className="col-span-2 flex gap-2">
                  ...
                  <button onClick={() => setEditId(null)}
                    className="text-xs font-medium px-3 py-1 rounded-full bg-gray-100 text-gray-600 hover:bg-gray-200 transition-colors">Cancel</button>
                </div>
              </>
            ) : (
              <>
