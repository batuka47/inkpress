...
export default function BreakingNewsManager({ items: initial, articles }: { items: Item[]; articles: Article[] }) {
  ...
  async function add() {
    ...
    const { data } = await db()
      .from("breaking_news")
      .insert({ text: text.trim(), active: true, link_article_id: articleId || null })
      .select("*, articles(title, slug)")
      .single();
    if (data) setItems((prev: Item[]) => [data, ...prev]);
    setText(""); setArticleId(""); setSearch("");
  }
  ...
  async function saveEdit(id: string) {
    const { data } = await db()
      .from("breaking_news")
      .update({ text: editText.trim(), link_article_id: editArticleId || null })
      .eq("id", id)
      .select("*, articles(title, slug)")
      .single();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border border-[--color-rule] rounded-xl p-5 space-y-4">
        ...
        <div>
          <label className="block text-xs font-semibold uppercase tracking-widest text-[--color-text-muted] mb-1">Text *</label>
          <input value={text} onChange={(e) => setText(e.target.value)}
            onKeyDown={(e) => e.key === "Enter" && add()}
            placeholder="Breaking news headline…" className={`w-full ${fieldCls}`} />
        </div>
        <div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex gap-2">
                  <button onClick={() => saveEdit(item.id)}
                    className="border border-black text-black text-xs font-medium px-4 py-1.5 rounded-lg hover:bg-black hover:text-white transition-colors">
                    Save
                  </button>
